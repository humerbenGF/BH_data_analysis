...
def q_change_over_phase_colored_by_li1_sustain_only(data_dir, num_phase_bins, min_num_crashes, max_num_crashes, q_psibar='min', show_plotly=False):
    ...
    li1_dict = load.load_json_file("plasma_data_and_parameters/internal_inductance/internal_inductance.json")
    ...
    for s in q_dict.keys():
        if s in crash_dict.keys() and s in sustainment_dict:
            if  type(str()) not in [type(sustainment_dict[s]), type(q_dict[s]), type(crash_dict[s]), type(li1_dict[s])]:
                ...
                if sustainment_dict[s] > 0 and len(crash_dict[s]['times']) > min_num_crashes and len(crash_dict[s]['times']) < max_num_crashes:
                    ...
                    for i in range(len(q_dict[s]['data'])):
                        ...
                        if cur_phase > 0.5 and j >= len(crash_dict[s]['times']):
                            ...
                            li1_array.append(li1_dict[s]['data'][i])
                    ...
                    for i in range(valid_q_points_shot - 1):
                        ...
                        li1_array_slopes.append((li1_array[-(i+1)] + li1_array[-i]) / 2)
    ...
    li1_array_slopes = [li1_array_slopes[i] for i in q_slope_indices_sorted_by_phase_sustain]
    ...
    fig1.add_trace(go.Scatter(
        ...
        marker=dict(
            color=li1_array_slopes,  # Use values from color_array
            colorscale='jet',  # Change this to your desired colormap ('viridis', 'jet', etc.)
            opacity=0.7,
            showscale=True,  # Show colorbar
            colorbar=dict(title="Internal Inductance")  # Label the colorbar
        ),
    ...
    if show_plotly:
        fig1.show()
    fig1.write_html(f"plots_github/GF_html_plot_viewer/PI3/axuv_crash_analysis_plots/q/dq_dp_colored_by_li1_{min_num_crashes}-{max_num_crashes}_crashes.html")
